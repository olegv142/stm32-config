...
static int flash_sec_erase(struct flash_sec const* sec)
{
	return flash_erase_sec(sec->no);
}

static int flash_sec_write(struct flash_sec const* sec, unsigned off, void const* data, unsigned sz)
{
	return flash_write(sec->base + off, data, sz);
}

static int flash_sec_write_bytes(struct flash_sec const* sec, unsigned off, void const* data, unsigned sz)
{
